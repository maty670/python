...
def gauss_seidel(matrix, b, initial_guess, K, tol=1e-15, max_iterations=100):
    n = len(matrix)
    ...
    for iteration in range(max_iterations):
        for i in range(n):
            sum_1 = sum(matrix[i][j] * x[j+1] for j in range(i))
...
solution,iteraciones,error = gauss_seidel(A, b, x0, K)
